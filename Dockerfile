...
RUN apt update && apt install -y brave-browser xvfb && apt-get clean
...
RUN pipenv install && pipenv install --system --deploy --ignore-pipfile && pipenv run playwright install
